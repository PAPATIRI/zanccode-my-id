<div class="pb-10">
    <div class="flex items-center font-sans gap-2 mb-4">
        ...
        <a wire:navigate href="{{route('admin.posts.create')}}"
           class="{{request()->routeIs('admin.posts.create') ? 'text-primary' : 'text-slate-700 dark:text-slate-300'}}">Tulis
            Sebuah Artikel</a>
    </div>
    <div class="bg-white dark:bg-boxdark rounded p-5">
        ...
        <div class="my-3 md:my-6">
            <form action="" wire:submit.prevent="createPost">
                                ...
                                <select wire:model="selectedCategories" id="select-category" multiple="multiple"
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{$category->id}}"
                                                class="text-body select-catetory">{{$category->title}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                ...
                <div class="flex items-center justify-end">
                    <button type="submit" class="bg-primary py-2 px-8 rounded text-slate-200">Simpan
                    </button>
                </div>
            </form>
        </div>
